...
#EGARCH : erreur dans varSet, n'arrive pas a trouver la fonction egarchSet
#m4 <- NULL
#v4 <- varSet(Egarch=list(ConstVar=0.2, Arch=c(0.4), Garch=c(0.6), Teta=0.2, Gamma=0.1))
#mod4 <- modelSet(condMean = m4,condVar = v4, condRes = r1)

#m4InitPoint <- NULL
#v4InitPoint <- varSet(Egarch=list(ConstVar=0.3, Arch=c(0.2), Garch=c(0.8), Teta=0.1, Gamma=0.3))
#mod4InitPoint <- modelSet(condMean = m4InitPoint,condVar = v4InitPoint, condRes = r1)
...
Param[1,1] = 0.4
Param[1,2] = 0.5
Param[1,3] = 0.0 # Ou rien je sais pas encore
#Model 2
Param[2,1] = 0.6
Param[2,2] = 0.5
Param[2,3] = 0.4
...
for (m in 1:NbModels) {
  ...
  print("Matrice d'erreur quadratiques des algo pour le modele m")
  M
  print("Matrice de temps d'execution moyen des algo pour les différents modeles")
  computingTime
