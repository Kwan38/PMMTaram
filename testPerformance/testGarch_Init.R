...
## In Garch model depending on the init point

.libPaths("/user/1/taram/Public/RLib/")
library(RRegArch)

# List of GSL algo
algoGSL.list <- c("gsl_ConjugateFR","gsl_ConjugatePR", "gsl_BFGS","gsl_Steepest",
...
res.errCumTmp <- 0
# Results
resListByMod <- list(distInitPoint = paramVectInit - paramGarch, ErrSqCum = c(), compTime = c())
res <- list(ConjugateFR = resListByMod, ConjugatePR = resListByMod, BFGS = resListByMod,
            Steepest = resListByMod, SimplexNM = resListByMod,
            SimplexNM2 = resListByMod, SimplexNM2Rand = resListByMod)

#Begin

print(paste("Test of model Garch with ", 1, " lags"))
# Creation of model Arch with 1 lags
var <- varSet(Garch = list(ConstVar = 0.1, Garch = paramGarch, Arch = c(0.1)))
mod <- modelSet(condMean = NULL, condVar = var, condRes = r)
for (n in 1:length(paramVectInit)) {
  print(paste("With init point ", paramVectInit[n], " against the true : ", paramGarch))
  #Set the init point
  varInit <- varSet(Garch = list(ConstVar = 0.1, Garch = paramGarch ,Arch = paramVectInit[n])) 
  ...
  for (s in 1:simuNb) {
    ...
    # Simulation
    garch.sim <- RegArchSim(1000,mod)
    # By algo
    for (a in 1:length(algoGSL.list)) {
      print(paste("and algo : ", algoGSL.list[a]))
      GSLAlgoParam <- setGSLParam(Algo = algoGSL.list[a])
      # Fit with this algo
      res.fit <- RegArchFit(model = mod, Yt = garch.sim$Yt, 
                            initPoint = modInit, AlgoParam = GSLAlgoParam)
      res.errCumTmp <- 0
      for (k in 2:2) {
        res.errCumTmp = res.errCumTmp + ((summary(res.fit))$coef[k] - paramGarch[k-1])^2
      }
...
#Display graph
colors <- c("black","lightblue", "darkblue", "darkgreen", "red", "pink", "cyan")
...
plot(res$ConjugateFR$distInitPoint, res$ConjugateFR$compTime,
     ...
     xlab = paste("Init point distance to ", paramGarch), ylab = "Time (s)",
     type = 'l', lwd = 2, ylim = c(0.025,0.05), col = colors[1])
...
# For error squared #NaN pour la plupart sauf simplex (Steepest s'arrete en 0)
plot(res$ConjugateFR$distInitPoint, res$ConjugateFR$ErrSqCum,
     ...
     xlab = paste("Init point distance to ", paramGarch), ylab = "Mean of Cum. Error Squared",
     type = 'l', lwd = 2, ylim = c(0.14,0.18), col = colors[1])
for (i in 2:length(algoGSL.list)) {
  lines(res[[i]]$distInitPoint, res[[i]]$ErrSqCum, type = 'l', lwd = 2, col = colors[i])
}
legend("bottomleft", legend = names(res), col = colors, pch = 8)
