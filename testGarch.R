...
paramVect <- c(0.2,0.5,0.8)
paramVectInit <- c(0.1,0.6,0.55)
# Mid result
res.errCumTmp <- 0
# Results
...
for (n in 1:length(paramVect)){
  ...
  var <- varSet(Garch = list(ConstVar = 0.1, Arch = c(0.1),Garch = paramVect[1:n]))
  mod <- modelSet(condMean = NULL, condVar = var, condRes = r)
  varInit <- varSet(Garch = list(ConstVar = 0.1, Arch = c(0.1), Garch = paramVectInit[1:n])) 
  ...
  for (s in 1:simuNb) {
    ...
    for (a in 1:length(algoGSL.list)) {
      ...
      res.errCumTmp <- 0
      for (k in 3:(n+2)) {
        res.errCumTmp = res.errCumTmp + ((summary(res.fit))$coef[k] - paramVect[k-2])^2
      }
